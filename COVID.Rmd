...
geom_line() +
theme_bw() +
labs(x = "Date", y = "Nombre de décès cumulés") +
scale_x_date(date_breaks = "week", date_labels = "%d %B", limits = c(as.Date("2020-03-01"),as.Date((Sys.Date()+2)))) +
scale_y_continuous(breaks=c(dir[(x),1],duk[(x),1], dg[(x),1], dus[(x),1], di[(x),1], df[(x),1], de[(x),1])) +
...
Sources : 2019 Novel Coronavirus COVID-19 (2019-nCoV) Data Repository by Johns Hopkins CSSE & www.santepubliquefrance.fr & data.gouv.fr
